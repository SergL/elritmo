...
?>
			...
			<div class="text-part">			
				...
				if (cat_is_ancestor_of(14,$catid)) { ?>
					
					<strong class="title">Украинские школы сальсы <span class="arrow">→</span> <?php echo single_cat_title('', false); ?></strong>	
					...
					<select onchange="window.open(this.options[this.selectedIndex].value,'_top')">
						<option selected value="<?php echo get_category_link(14); ?>">Все города Украины...</option>
							...
							{
								$category_id=get_cat_id($city_name);
								echo "<option value='".get_category_link($category_id)."'";
								
								
								if (get_query_var('cat')==$category_id)
								{
									echo " selected ";
								};
				...
				<?php // Description for category from [else] pages //
					global $wpdb;
					$catid="cat".$catid;
					$cat_text=$wpdb->get_results("SELECT * FROM `wp_posts` WHERE ((post_title='$catid') and (post_status='publish'))");			
					echo apply_filters('the_content',$cat_text[0]->post_content);
				?>
