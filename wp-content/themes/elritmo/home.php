			...
			<div class="sidebar-left">
				...
				<?php if ($last_photo_article->have_posts()) : ?>
					<?php while ($last_photo_article->have_posts()) : $last_photo_article->the_post(); ?>
								...
								<small class="gray"><?php echo maxsite_the_russian_time(the_time('j F Y')); ?> </small>							
								</p>
					<?php endwhile; ?>
				<?php endif; ?>			
				...
				<?php 
					...
					if ($r->have_posts()) :				
						...
						while ($r->have_posts()) : $r->the_post(); ?>						 
							...
							<p class="prewiev-news"><b><small><?php the_time('j.n.Y') ?></small></b>&nbsp;&nbsp;<small class="gray">Раздел: <?php the_category(', '); ?></small><br/><a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?> </a></p>
