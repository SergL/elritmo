...
function wp_cumulus_createflashcode( $widget=false, $atts=NULL ){
	...
	if( $options['compmode']!='true' ){
		...
		$flashtag .= '</p><p>Для показа облака необходим Flash Player 9 или выше.</p></div>';
		...
		$flashtag .= '<p>Для просмотре облака меток требуется Flash Player 9 или выше.</p>';
		$flashtag .= '</object>';
	}
	return $flashtag;
}
...
function wp_cumulus_options() {	
	...
	echo '<td><input type="text" name="tcolor" value="'.$options['tcolor'].'" size="8"></input> Цвет градиента (опционально): <input type="text" name="tcolor2" value="'.$options['tcolor2'].'" size="8"></input>  Цвет подсветки (опционально): <input type="text" name="hicolor" value="'.$options['hicolor'].'" size="8"></input><br />6-значный hex-код цвета без префикса # в начале (000000 - черный, ffffff - белый)</td></tr>';
	// background color
	...
	echo '></input><br />Включение опции wmode-transparent во Flash (то есть прозрачный фон - не рекомендуется)</td></tr>';
	// speed
	...
	echo '<td><input type="text" name="args" value="'.$options['args'].'" size="60"></input><br />Строка с параметрами для функции wp_tag_cloud (смотрите в codex описание функции)<br /><br /><strong>Примеры использования</strong><br />number=20 - ограничивает число тегов 20-ю<br />smallest=5&largest=50 - указывает размер шрифта<br /><br /><strong>Возможные проблемы</strong><ul><li>На данный момент, параметры \'units\', \'orderby\' и \'order\' не поддерживаются.</li><li>Попытка задать параметр \'format\' на любое значение кроме \'flat\' вызовет ошибку плагина.</li></ul></td></tr>';	
	// compatibility mode
	echo '<tr valign="top"><th scope="row">Использовать режим совместимости?</th>';
	echo '<td><input type="checkbox" name="compmode" value="true"';
	if( $options['compmode'] == "true" ){ echo ' checked="checked"'; }
	echo '></input><br />Включение данной опции изменяет способ встраивания флешки в ваш шаблон. Используйте эту функцию, если у вас наблюдаются проблемы с правильным отображением облака тегов (эффект применяется и к виджету).</td></tr>';
	// show regular tag in alternate content?
	echo '<tr valign="top"><th scope="row">Показывать обычное HTML облако тегов?</th>';
	echo '<td><input type="checkbox" name="showwptags" value="true"';
	if( $options['showwptags'] == "true" ){ echo ' checked="checked"'; }
	echo '></input><br />Включение данной опции активирует создание HTML облака тегов (на секунду-другую ваш посетитель будет видеть обычное HTML облако, а только потом загрузится flash облако).</td></tr>';
	// close stuff
...
function widget_init_wp_cumulus_widget() {
	...
	function wp_cumulus_widget_control() {
		...
		?>
			...
			<p><label for="wpcumulus_widget_tcolor2"><?php _e('Цвет градиента (опционально)'); ?> <input class="widefat" id="wpcumulus_widget_tcolor2" name="wpcumulus_widget_tcolor2" type="text" value="<?php echo $tcolor2; ?>" /></label></p>
			<p><label for="wpcumulus_widget_hicolor"><?php _e('Цвет подсветки (опционально):'); ?> <input class="widefat" id="wpcumulus_widget_hicolor" name="wpcumulus_widget_hicolor" type="text" value="<?php echo $hicolor; ?>" /></label></p>
			...
			<p><label for="wpcumulus_widget_trans"><input class="checkbox" id="wpcumulus_widget_trans" name="wpcumulus_widget_trans" type="checkbox" value="true" <?php if( $trans == "true" ){ echo ' checked="checked"'; } ?> > Прозрачный фон (не рекомендуется)</label></p>
